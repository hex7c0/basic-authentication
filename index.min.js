...
function end_check_file(auth, hash, file) {
    ...
    for (var i = 0; i < ii; ++i) if (request.test(input[i]) && input[i].substring(request.source.length - 1) === psw) return !1;
    return !0;
}

function basic_legacy(req, force) {
    var auth;
    return !0 === force ? (auth = new Buffer(req, "base64").toString("utf8"), (auth = auth.match(basic)) && auth[1] ? {
        user: auth[1],
        password: auth[2]
...
function authentication(opt) {
    function basic_medium(req, res, next) {
        ...
        return "" !== auth ? !0 === check(auth, my.hash, my.file) ? !0 === setHeader(res, "WWW-Authenticate", my.realms) ? end_work(err, next, 401) : null : "" === my.agent || my.agent === req.headers["user-agent"] ? end_work(err, next) : end_work(err, next, 403) : (res.writeHead(401, my.realm), 
        res.end());
    }
    ...
    return my.suppress && (err = function() {}), !1 === options.ending ? basic_medium : basic_big;
}
