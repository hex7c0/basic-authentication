...
function basic_legacy(req, force) {
    var auth;
    return force === !0 ? (auth = new Buffer(req, "base64").toString("utf8"), auth = auth.match(basic), 
    {
        user: auth[1],
        password: auth[2]
    }) : req.headers && (auth = req.headers.authorization) && (auth = auth.match(reg)) && auth[1] && (auth = new Buffer(auth[1], "base64").toString("utf8").match(basic)) ? {
        user: auth[1],
        password: auth[2]
    } : Object.create(null);
}
...
function authentication(opt) {
    function wrapper_medium() {
        ...
        return my.suppress && (err = end_empty), function(req, res, next) {
            ...
            return "" !== auth ? check(auth, my.hash, my.file) === !0 ? void (setHeader(res, "WWW-Authenticate", my.realms) && end_work(err, next, 401)) : "" === my.agent || my.agent === req.headers["user-agent"] ? end_work(err, next) : end_work(err, next, 403) : (res.writeHead(401, my.realm), 
            res.end());
        };
    }
    function wrapper_big() {
        ...
        return my.suppress && (err = end_empty), function(req, res, next) {
            var auth = basic_small(req);
            return "" !== auth ? check(auth, my.hash, my.file) === !0 ? void (setHeader(res, "WWW-Authenticate", my.realms) && end_work(err, next, 401, res)) : "" === my.agent || my.agent === req.headers["user-agent"] ? end_work(err, next) : end_work(err, next, 403, res) : (res.writeHead(401, my.realm), 
            res.end(http[401]));
        };
    }
    ...
    return options.ending === !1 ? wrapper_medium() : wrapper_big();
}
