...
function basic_legacy(req, force) {
    var auth;
    return force === !0 ? (auth = new Buffer(req, "base64").toString(), auth = auth.match(/^([^:]*):(.*)$/), 
    {
        user: auth[1],
        password: auth[2]
    }) : void 0 !== req.headers && void 0 !== (auth = req.headers.authorization) && (auth = auth.match(reg)) && void 0 !== auth[1] && (auth = new Buffer(auth[1], "base64").toString().match(/^([^:]*):(.*)$/), 
    void 0 !== auth) ? {
        user: auth[1],
        password: auth[2]
    } : Object.create(null);
...
try {
    var reg = new RegExp(/^Basic (.*)$/), http = require("http").STATUS_CODES, crypto = require("crypto").createHash, setHeader = require("setheaders");
